...
// ---- effectiveGroups (shared ordered group list) ----

import { effectiveGroups } from '../flowMetrics'
...
describe('effectiveGroups', () => {
  ...
  it('declared groups first, then legacy sectionName, deduped', () => {
  ...
  it('falls back to flow sections when nothing is declared; ignores empty', () => {
    expect(effectiveGroups([], flows)).toEqual(['Dock', 'ASRS'])
  })
})
