  ...
  return (
    <svg viewBox={`0 0 ${VIEW_W} ${totalH}`} className="rv-fd" role="img" aria-label="Material flow map" preserveAspectRatio="xMinYMin meet">
      ...
      {links.map((l, i) => {
        ...
        return (
          <g key={`l${i}`}>
            <path d={path(l)} className="rv-fd-link" fill="none"
              strokeWidth={1.5 + Math.min(3.5, l.thru / 18)} markerEnd="url(#rv-fd-arrow)" />
            <rect x={mx - pw / 2} y={my - 19} width={pw} height={16} rx={8} className="rv-fd-pill" />
      ...
      {dests.map((d, i) => (
        <g key={`d${i}`}>
          ...
          <rect x={d.node.x} y={d.node.y} width={3} height={d.node.h} rx={1.5} className="rv-fd-dest-tick" />
