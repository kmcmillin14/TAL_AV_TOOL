...
describe('change-log fields round-trip', () => {
  it('carries new intake fields through the envelope + reparses', () => {
    ...
    // Every change-log field must survive the envelope untouched (guards against
    // normalizeForPort/spread silently dropping or overwriting any of them).
    const p = env.project
    expect(p.submissionType).toBe('dealer')
    expect(p.dealershipName).toBe('Acme Lift')
    expect(p.dealerRep).toBe('Sam')
    expect(p.unitLoadTypes).toEqual(['Standard Pallet', 'Tote'])
    expect(p.typicalUnitType).toBe('Standard Pallet')
    expect(p.driveAisleWidthFt).toBe(10)
    expect(p.rackingAisleWidthFt).toBe(6)
    expect(p.minAisleWidthFt).toBe(6)
    expect(p.sharedTrafficTypes).toEqual(['Pedestrians'])
    expect(p.guidanceType).toBe('wire')
    expect(p.hazardZoneClassification).toBe('Zone 1')
    expect(p.restApiAvailable).toBe('yes')
    expect(p.barcodeScanningRequired).toBe(true)
    expect(p.wmsInterfaceType).toBe('rest_api')
    expect(p.taggingScanMethod).toBe('barcode')
    expect(p.hasExistingAutomation).toBe(true)
    expect(p.existingAutomationInterop).toBe('shared aisles')
    expect(p.currentHeadcount).toBe(4)
    expect(p.dwellTimeMin).toBe(2)
    expect(p.chargingStrategyPreference).toBe('opportunity')
  })
})
