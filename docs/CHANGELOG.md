...
- Questionnaire UI: §01 submitter routing (required), §03 multi-select load types, §04
  dwell/charging/lift fields, §05 aisle split + shared traffic + VNA guidance + ramps +
  temp-range gating, §06 CAD single home, §07 per-flow distance type, §09 hazard/barcode/
  WMS cluster, §12 existing-automation toggle + headcount; status-bar "Required" marker;
  dual PDF + JSON export. Backend items (file upload, auto-email, signed URLs) intentionally
  deferred. See docs/questionnaire-interop.md.
...
- **Fix B — walkthrough ends with wipe-and-start action:** `Guide` type gains optional `finishLabel` and `finishAction: 'discardSampleAndStart'`. On the last step, the primary button shows `finishLabel` (else `'Get started'`). When clicked with the action, the engine calls `deleteProject(sampleProjectId)` then `findOrCreateEntryProject()` and navigates to `/step0`. A new 9th step ("Your turn") warns the user before the wipe fires; button label is `'Start your application'`. `projectId` is threaded through `sessionStorage['tal:guideState']` and the `GUIDE_EVENT` detail so the engine always knows which project to delete.
- **Fix C — scrimless active-section highlight:** `.tour-scrim` removed entirely from the render and the CSS. `.tour-root` is now `pointer-events: none` (page fully interactive); `.tour-card` restores `pointer-events: auto`. `.tour-highlight` redesigned as `outline: 2px solid var(--accent); outline-offset: 2px; border-radius: 8px; background: color-mix(in srgb, var(--accent) 6%, transparent)` — no z-index hacks. Card docked **bottom-right** (`right: 24px; bottom: 24px`) on all screens; `.is-centered` class kept in markup but carries no style delta (single consistent position).
- **Fix D — Company A generic DC sample:** `src/content/samples/toyota-project.json` deleted; replaced by `src/content/samples/company-a-project.json` — projectName "Sample — Company A Distribution Center", customerName "Company A", facilityLocation "Distribution Center — Anytown, USA"; GMA 48×40 pallet at 1,800 lbs (`palletSubtype: "GMA (48×40)"`, `loadWidthIn: 40`); flow groups renamed Receiving & Putaway / Replenishment / Outbound with generic DC origins/destinations (Receiving Dock → Reserve Racking / Reserve Racking → Forward Pick / Forward Pick → Pack Line 1 & 2 / Pack Out → Staging / Staging → Shipping Dock). All walkthrough copy in `GuidedTour.tsx` updated (no Toyota/automotive/lineside references). Step 0 pill updated to "Company A (guided)". Test file `sampleProject.test.ts` updated (filename, describe block, group names, new GMA dimension assertion). Toyota logo/branding of the app itself untouched.
...
- **'Custom' transfer starts at 0s.** The Custom method is engineer-defined:
  ...
  Custom. Single-method cells share the select's box metrics so all Transfer
  cells align.
